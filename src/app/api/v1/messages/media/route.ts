...
import { enqueueMediaJob, isQueueUnavailableError } from "@/server/queue/queue";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const input = mediaMessageSchema.parse(await request.json());
    const sessionId =
    ...
    statusStore.addJobLog({
      ...
      type: "media",
      status: "queued",
      message: `Queued media to ${input.to}`,
    });
